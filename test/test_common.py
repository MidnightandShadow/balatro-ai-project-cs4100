...
class TestCommon:
    ...
    def test_score_card(self):
        ...
        assert (
            self.king_of_clubs.score()
            == self.queen_of_diamonds.score()
            == self.jack_of_hearts.score()
            == self.ten_of_spades.score()
            == 10
        )
    ...
    def test_score_hand(self):
        ...
        assert (
            self.scored_straight_hand.score()
        ...
        assert (
            ScoredHand(
                PokerHand.PAIR,
                [self.three_of_spades, self.three_of_clubs],
                [self.ace_of_spades],
            ).score()
    ...
    def test_hand_to_scored_hand_flush(self):
        assert hand_to_scored_hand(self.flush_hand) == ScoredHand(
            PokerHand.FLUSH, self.flush_hand, []
        )
    ...
    def test_hand_to_scored_hand_straight_flush(self):
        ...
        assert hand_to_scored_hand(straight_flush_hand) == ScoredHand(
            PokerHand.STRAIGHT_FLUSH, straight_flush_hand, []
        )
    ...
    def test_hand_to_scored_hand_we_are_counting_royal_flush_as_straight_flush(self):
        ...
        assert hand_to_scored_hand(royal_flush_hand) == ScoredHand(
            PokerHand.STRAIGHT_FLUSH, royal_flush_hand, []
        )

    def test_hand_to_scored_hand_full_house(self):
        scored_full_house_hand = ScoredHand(
            PokerHand.FULL_HOUSE, self.full_house_hand, []
        )
        ...
        assert hand_to_scored_hand(not_full_house_hand) == ScoredHand(
    ...
    def test_hand_to_scored_hand_four_of_a_kind(self):
        ...
        assert hand_to_scored_hand(four_of_a_kind_hand) == ScoredHand(
            PokerHand.FOUR_OF_A_KIND, four_of_a_kind_hand, []
        )

        assert hand_to_scored_hand(four_of_a_kind_hand_with_extra_card) == ScoredHand(
            PokerHand.FOUR_OF_A_KIND, four_of_a_kind_hand, [self.ace_of_spades]
        )
    ...
    def test_hand_to_scored_hand_two_pair(self):
        ...
        assert hand_to_scored_hand(two_pair_hand) == ScoredHand(
            PokerHand.TWO_PAIR, two_pair_hand[0:4], [two_pair_hand[4]]
        )
    ...
    def test_hand_to_scored_hand_three_of_a_kind(self):
        ...
        assert hand_to_scored_hand(three_of_a_kind_hand) == ScoredHand(
            PokerHand.THREE_OF_A_KIND,
            three_of_a_kind_hand[0:3],
            [three_of_a_kind_hand[3]],
        )
    ...
    def test_hand_to_scored_hand_pair(self):
        ...
        assert hand_to_scored_hand(pair_hand) == ScoredHand(
            PokerHand.PAIR, pair_hand[0:2], pair_hand[2:4]
        )
    ...
    def test_hand_to_scored_hand_high_card(self):
        ...
        assert hand_to_scored_hand(high_card_hand) == ScoredHand(
            PokerHand.HIGH_CARD,
            [self.ace_of_spades],
            [
                self.king_of_clubs,
    ...
    def test_group_cards_by_attribute(self):
        ...
        assert group_cards_by_attribute(six_cards, CardAttribute.SUIT) == [
            ...
            [self.ten_of_clubs, self.six_of_clubs],
            [self.ten_of_hearts],
            [self.ten_of_diamonds],
        ]
